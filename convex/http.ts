...
http.route({path:"/api/incidents",method:"GET",handler:httpAction(async(ctx,req)=>{const url=new URL(req.url);const projectId=url.searchParams.get("projectId")??undefined;const limit=Number(url.searchParams.get("limit")??50);return jsonResponse({data:await ctx.runQuery(api.incidents.list,{projectId,limit:Math.min(Math.max(limit,1),100)})})})});
http.route({path:"/api/incidents",method:"POST",handler:httpAction(async(ctx,req)=>{try{const body=parseCreateIncidentRequest(await req.json());const id=await ctx.runMutation(api.incidents.create,{projectId:body.projectId,source:body.source,service:body.service,severity:body.severity,configuredMode:body.configuredMode,idempotencyKey:body.idempotencyKey});return jsonResponse({data:{id}},201)}catch(error){return jsonResponse({error:error instanceof Error?error.message:"INVALID_REQUEST"},400)}})});
http.route({path:"/api/incidents/detail",method:"GET",handler:httpAction(async(ctx,req)=>{const id=new URL(req.url).searchParams.get("incidentId");if(!id)return jsonResponse({error:"incidentId is required"},400);return jsonResponse({data:await ctx.runQuery(api.dashboard.incidentDetail,{incidentId:id})})})});
http.route({path:"/api/overview",method:"GET",handler:httpAction(async(ctx,req)=>{const projectId=new URL(req.url).searchParams.get("projectId")??undefined;return jsonResponse({data:await ctx.runQuery(api.dashboard.overview,{projectId})})})});
