import { z } from "zod";
export const diagnosisOutput = z.object({rootCause:z.string().min(1),confidence:z.number().min(0).max(1),evidence:z.array(z.object({source:z.enum(["log","deployment","commit","code","runbook","research"]),summary:z.string(),ref:z.string()}).strict()).min(2),affectedSurfaces:z.array(z.string()),risk:z.enum(["low","medium","high"]),recommendedRepair:z.string(),requiredFiles:z.array(z.string()),dependencyChange:z.boolean(),migrationChange:z.boolean(),secretChange:z.boolean()}).strict();
