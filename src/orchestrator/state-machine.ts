...
const transitions: Record<IncidentStatus, readonly IncidentStatus[]> = {
  ...
  PATCHING:["PATCH_REVIEW","RETRYING","ESCALATED"], PATCH_REVIEW:["PR_READY","ESCALATED"],
...
export function expectedPath(mode:Mode):IncidentStatus[]{
  ...
  if(mode==="PR_APPROVAL") return ["DETECTED","DIAGNOSING","DIAGNOSIS_REVIEW","PATCHING","PATCH_REVIEW","PR_READY"];
  return ["DETECTED","DIAGNOSING","DIAGNOSIS_REVIEW","PATCHING","PATCH_REVIEW","PR_READY"];
}
